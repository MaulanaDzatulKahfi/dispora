    ...
    <div class="flex bg-green-600 p-4 shadow justify-between">
        <div class="text-xl text-white">
            <h3 class="font-bold pl-2">
                <a href="{{ route('perting.index') }}" class="hover:text-gray-200">{{ $fakultas->perting->name }}</a> / <a href="{{ route('fakultas.index', $fakultas->perting->id) }}" class="hover:text-gray-200">Fakultas</a>
            </h3>
        </div>
    </div>
        ...
        <div class="container mx-auto">
            <div class="max-w-xl mx-auto mt-10 bg-white p-5 rounded-md shadow-sm">
                <div class="text-center">
                    <h1 class=" text-xl font-semibold text-gray-700 dark:text-gray-200">Form Edit Fakultas</h1>
                </div>
                <div class="m-7">
                    <form action="{{ route('fakultas.update', $fakultas->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            value="{{ $fakultas->name }}" placeholder="Nama Fakultas" required/>
                            @error('name')
                        ...
                        <div class="mb-4">
                            ...
                            name="status"
                            class="w-full px-3 py-1 border border-gray-300 rounded-md text-gray-600 focus:outline-none focus:ring focus:ring-indigo-100 focus:border-indigo-300"
                            value="{{ $fakultas->status }}" placeholder="Status" required/>
                            @error('status')
                                <p class="text-xs italic text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-1 mt-10 flex justify-between">
                            <a href="{{ route('fakultas.index', $fakultas->perting->id) }}" class="px-4 py-2 rounded-md bg-gray-200 focus:bg-gray-400 focus:outline-none">Kembali</a>
